...
public class Result9 extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_result9);
        textView=findViewById(R.id.book);
        ArrayList<Flight> flightArrayList=new ArrayList<>();
        FlightList flightList=new FlightList();
        for (int i=0;i<flightList.getSize();i++){
            if (flightList.getFlightByNum(i).getDeparture().equals("madrid")&&flightList.getFlightByNum(i).getArrival().equals("copenhagen")){
                flightArrayList.add(flightList.getFlightByNum(i));
            }
        }

        recyclerView=findViewById(R.id.FlightsRecView9);
